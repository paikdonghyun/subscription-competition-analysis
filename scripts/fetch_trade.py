...
REGIONS = {
    ...
    '41250': '경기 동두천시',    '41273': '경기 안산 단원구', '41271': '경기 안산 상록구',
...
def main():
    ...
    # 주소 매칭용 regions.json 생성 (apt.html 실거래가 조회에 사용)
    regions_map = {}
    for code, region_nm in REGIONS.items():
        parts = region_nm.split(' ', 1)
        if len(parts) == 2:
            regions_map[code] = {'sido': parts[0], 'gu': parts[1]}
    with open(OUT_DIR / 'regions.json', 'w', encoding='utf-8') as f:
        json.dump(regions_map, f, ensure_ascii=False, separators=(',',':'))

    print(f'\n완료: {len(summary)}개 지역 / summary.json · regions.json 저장')
